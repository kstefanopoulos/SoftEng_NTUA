...
public String registerUserAccount(Model model, @ModelAttribute("parentdto") @Valid ParentDTO accountDto, 
   ...
   /* if (result.hasErrors()) { */ 
    ...
    if (registered==null)
    	return "UnsuccessfulRegistrationPage";
    redirectAttributes.addFlashAttribute("flashUser", registered);
    return "/littlecherries/registerSuccessful"; /*} */ 
}
...
@RequestMapping(value="/littlecherries/administrators/viewblockedusers" , method = RequestMethod.GET)
public String AdministratorViewBlockedUsers(Model model){
			if (userCredentials == null)
				return "registration";
			administrator adm = ad.getΑnAdmin(userCredentials.getEmail());
			if (adm!=null){
				...
				int blockedparents = 0 ;
				int blockedorganizers = 0; 
				int blockedadmins = 0 ; 
				int blockedusers = 0; 
				String email =""; 
				int type; 
				...
				for ( administrator admin : ads){
					...
					if( admin.getRestrictions()!= 0 )
						email = admin.getEmail(); 
					    type = 2 ;
					    users nu = new users(email,type);
						list.add(nu); 
						blockedadmins++; 
	            ...
	            for ( parent parent : pars){
					...
					if( parent.getRestrictions()!= 0 )
						email = parent.getPemail(); 
					    type = 0 ;
					    users nu = new users(email,type);
						list.add(nu); 
						blockedparents++; 
	            ...
	            for ( organizer organizer : orgs){
	       					...
	       					if( organizer.getRestrictions()!= 0 )
	       						email = organizer.getOemail(); 
	       					    type = 0 ;
	       					    users nu = new users(email,type);
	       						list.add(nu); 
	       						blockedorganizers++; 
	       				}
	       		blockedusers = blockedorganizers+ blockedparents + blockedadmins ;  
	            model.addAttribute("blockedorganizers",blockedorganizers);
	            model.addAttribute("blockeparents", blockedparents); 
	            model.addAttribute("blockedadmins",blockedadmins);
	            model.addAttribute("blockeuser", blockedusers); 
	            model.addAttribute("list", list);
	            
				return "viewblockedusers" ; 
...
public String AdministratorViewRestriction(Model model){
			...
			if (adm!=null){
				...
				List<restrictions> list = new ArrayList<restrictions>(); 
				for(restrictions r : R){
					list.add(r); 
 				}
				int a = list.size();  
				
				model.addAttribute("restrict", list); 
